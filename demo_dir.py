...
if __name__ == '__main__':
    ...
    testmask = np.zeros((256, 256), dtype=np.float32)
    ...
    for each in images:
        ...
        # rsz_img = np.zeros((256, 256, 3))
        # rsz_img[0:img.shape[0], 0:img.shape[1]] = img[0:img.shape[0], 0:img.shape[1]]

        for j in range(3):
            # testimg[:, :, j] = np.clip(img[:, :, j].astype(np.float32) / 255.0, 0.0, 1.0)
            testimg[0:img.shape[0], 0:img.shape[1], j] = np.clip(img[:, :, j].astype(np.float32) / 255.0, 0.0, 1.0)
        ...
        mask = io.imread(os.path.join(params.mask_dir, each[:-4] + '.png'))
        # depth_gt = cv2.resize(depth_gt, (256, 256), interpolation=cv2.INTER_NEAREST)
        testmask[0:mask.shape[0], 0:mask.shape[1]] = mask[:, :, 0].astype(np.float32)
        # mask_batch[i] = depth_gt > 1e-3
        mask_batch[i] = testmask > 1e-3
        i = i + 1
        ...
        for i in range(params.batch_size):
            util.depth2mesh(basic_depth_[i], mask_batch[i], os.path.join(params.output_dir, each[:-4] + '_basic_depth'))
            util.depth2mesh(pred_detail_depth_[i], mask_batch[i], os.path.join(params.output_dir, each[:-4] + '_detail_depth'))
            util.depth2mesh(regressed_depth_[i], mask_batch[i], os.path.join(params.output_dir, each[:-4] + '_regress_depth'))
