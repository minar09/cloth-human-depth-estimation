...
def projector(depth,intrinsic,mask,filename):
    fx = intrinsic[0,0]
    fy = intrinsic[1,1]
    cx = intrinsic[0,2]
    cy = intrinsic[1,2]
    ...
    for i in range(h):
        for j in range(w):
            ...
            X = Z * (i - cy) / fx
            Y = Z * (j - cx) / fy
            f.write('v '+str(float(X))+' '+str(float(Y))+' '+str(float(Z))+'\n')

    threshold = 0.04

    plt.figure()
    plt.imshow(depth, aspect='auto', cmap=plt.get_cmap('jet'))
    plt.show()
    ...
    for i in range(h-2):
        for j in range(w-2):
            ...
            if np.max(dy_u)<threshold and np.max(dx_l) < threshold and np.max(dy_d) < threshold and np.max(dx_r) < threshold and mask[i,j]:
                f.write('f ' + str(int(i*w+j))+' '+ str(int(i*w+j+1))+' '+ str(int((i+1)*w+j))+'\n')
                f.write('f ' + str(int((i+1)*w+j+1))+' '+ str(int((i+1)*w+j))+' '+ str(int(i*w+j+1))+'\n')
    f.close()
    return
...
def cropfor3d(img, center, scale, rot, res, method):
    ...
    if len(img.shape) > 2:
        ...
        newImg = np.zeros([newDim[0],newDim[1],newDim[2]]).astype(np.uint8)
        ht = img.shape[0]
        wd = img.shape[1]
    else:
...
def cropfor3d_intrinsic(img, center, scale, rot, res, method):
    ul = transform([1, 1], center, scale, 0, res, True)
    br = transform([res, res], center, scale, 0, res, True)
    pad = np.floor(np.linalg.norm(ul - br) / 2 - (br[0] - ul[1]) / 2).astype(np.int64)
    ...
    output['newDim'] = newDim
    output['center'] = [(br[1] + ul[1])/2,(br[0] + ul[0])/2]
    output['scale'] = [res/newDim[0], res/newDim[1]]
    return output
