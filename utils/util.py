...
def cropforintrisic(oldcx, oldcy, oldfx, oldfy, height, width, crop_ci, crop_cj, scale):
    ...
    """
    cx = oldcx + float(width - 1) / 2 - crop_cj
    cy = oldcy + float(height - 1) / 2 - crop_ci
    fx = oldfx
    fy = oldfy
...
def relative(depth, dPelvis, step, Zres):  # halfrange
    ...
    median = np.median(depth[fgix])
    out[fgix] = np.clip(depth[fgix] - median, lowB, upB)
    out[bgix] = 0.6
...
def compute_depth_err(batchindex, pred_depth_, depth_batch, mask_batch, datatype):
    ...
    for i in range(batch_num):
        ...
        depth_shift = np.multiply((np.copy(pred_depth_[i]) + offset).reshape([256, 256]), mask)
        errmap = np.absolute(depth_shift - gtdepth)
        ...
        im.set_data(colors)
        plt.savefig('/home/sicong/Downloads/out/' + str(batchindex) + '_' + str(i) + '_' + datatype + '.png')
...
def new_compute_depth_err_auc(batchindex, pred_depth_, depth_batch, refined_mask_out, mask_batch, fg_mask, datatype, bin_num = 100):
    res = 160
    offset_range = 0.40
    hist_range = 0.5
    ...
    batch_num = pred_depth_.shape[0]
    err = np.zeros([batch_num, res], dtype=np.float32)
    err_out = np.zeros([batch_num,res],dtype = np.float32)
    histout = np.zeros([bin_num], np.float32)
    bestoffset = np.zeros([batch_num], np.int32)
    depth_shift_arr = np.zeros([bin_num],np.float32)
    output = {}


    ## align two depth images first
    for i in range(batch_num):
        mask = fg_mask[i].astype(np.float32)
        gtdepth_masked = np.multiply(np.copy(depth_batch[i]), mask)
        pnum = np.sum(mask)
        median_offset = np.median(gtdepth_masked[mask>0.5]) - np.median(pred_depth_[i][mask>0.5])
        depth_shift_arr[i] = median_offset
        for j in range(res):
            offset = (j - (res / 2)) * (offset_range / res)
            depth_shift = np.multiply((np.copy(pred_depth_[i]) + offset + median_offset).reshape([256, 256]), mask)
            errmap = np.absolute(depth_shift - gtdepth_masked)
            err[i, j] = np.sum(errmap) / pnum
        bestoffset[i] = np.argmin(err[i, :])


    ## use gt_mask to compute histgram and aver error again
    for i in range(batch_num):
        mask = mask_batch[i].astype(np.float32)
        refined_mask = refined_mask_out[i]
        gtdepth = np.multiply(np.copy(depth_batch[i]), mask)
        pnum = np.sum(mask)
        minloc = bestoffset[i]
        ...
        depth_shift = np.multiply((np.copy(pred_depth_[i]) + offset + median_offset).reshape([256, 256]), mask)
        depth_shift[refined_mask == False] = 0.0
        errmap = np.absolute(depth_shift - gtdepth)
        histout += \
            np.histogram(errmap.reshape(256 * 256), bins=bin_num, range=(0.0, hist_range), weights=mask.reshape(256 * 256),
                         density=False)[0]
        err_out[i] = np.sum(errmap) / pnum
        ...
        fig.colorbar(im, cax=cax, orientation='vertical')
        im.set_data(colors)
        plt.savefig('./error_map/' + str(batchindex) + '_' + str(i) + ' ' + datatype + '.png')
        plt.close()
...
def restore_rgb(img, meanrgb_dir):
    meanstd = load_obj(meanrgb_dir)
    for j in range(3):
...
def masked_gasussion_filter(depth, sigma=1, size=5):
    kernel = np.fromfunction(lambda x, y: (1 / (2 * math.pi * sigma ** 2)) * math.e ** ((-1 * ((x - (size - 1) / 2) ** 2 + (y - (size - 1) / 2) ** 2)) / (2 * sigma ** 2)), (size, size))
    h_size = int((size - 1)/2)
    new_depth = np.zeros_like(depth)
    for i in range(depth.shape[0] - size + 1):
        for j in range(depth.shape[1] - size + 1):
            patch_x = i + h_size
            patch_y = j + h_size
            localpatch = np.copy(depth[patch_x-h_size:patch_x+h_size+1,patch_y-h_size:patch_y+h_size+1])
            localpatch_diff_bool = np.abs(localpatch - depth[patch_x,patch_y]) < 0.10
            weight = kernel * localpatch_diff_bool
            new_depth[patch_x, patch_y] = np.sum(localpatch * weight) / np.sum(weight)

    return new_depth

def getheat(segmentation, joints_3d, bn):
    # segmentation = np.empty((segmentationori.shape[0], 64, 64),
    #                         dtype=np.float32)
    # if segmentationori.shape[1] != 64:
    #     for i in range(segmentation.shape[0]):
    #         segmentation[i] = cv2.resize(segmentationori[i], (64, 64), interpolation=cv2.INTER_NEAREST)
    # else:
    #     segmentation = segmentationori
    seg_heat = np.empty((segmentation.shape[0], segmentation.shape[1], segmentation.shape[2], 15),
                            dtype=np.float32)
    joints_heat = np.empty((segmentation.shape[0], 64, 64, 304),
                            dtype=np.float32)

    # plt.figure()
    # plt.imshow(segmentation[0, :, :], aspect='auto', cmap=plt.get_cmap('jet'))
    # plt.show()

    for i in range(bn):
        seg_heat[i] = extent(segmentation[i], 15)
        # for k in range(15):
        #     plt.figure()
        #     plt.imshow(seg_heat[j,:,:,k], aspect='auto', cmap=plt.get_cmap('jet'))
        #     plt.show()

        sigma_2d = 2
        size_z = 2 * math.floor((6 * sigma_2d * 19 / 64 + 1) / 2) + 1
        out = np.zeros([64, 64, 304])
        for j in range(16):
            # if joints2d[1,j] >= img_full.shape[0] or joints2d[0,j] >=img_full.shape[1] or joints2d[1,j]<0 or joints2d[0,j]<0:
            # continue
            z = math.floor(joints_3d[i,j,2] - 9.0)
            pt = [math.floor(joints_3d[i,j,1]),math.floor(joints_3d[i,j,0])]
            out[:, :, j * 19: (j + 1) * 19] = Drawguassian3D(out[:, :, j * 19: (j + 1) * 19], pt, z, sigma_2d, size_z)
        joints_heat[i] = out

    return seg_heat, joints_heat

def getheat_surreal(segmentationori, joints_3d, bn):
    segmentationori = segmentationori - 1
    segmentation = np.empty((segmentationori.shape[0], 64, 64),
                            dtype=np.float32)
    if segmentationori.shape[1] != 64:
        for i in range(segmentation.shape[0]):
